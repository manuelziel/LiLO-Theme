 ...
 */
function lilo_get_svg( $icon = null ) {
 ...
 */
function lilo_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'primary' === $args->theme_location || 'secondary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . lilo_get_svg( 'expand' );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'lilo_dropdown_icon_to_menu_link', 10, 4 );
 ...
 * @return string $svg SVG markup.
 */
function lilo_get_social_svg( $icon = null ) {
	// Return early if no icon was defined.
	if ( empty( $icon ) ) {
		return;
	}
 ...
 */
function lilo_social_icons_menu( $item_output, $item, $depth, $args ) {
	...
	$social_icons = lilo_supported_social_icons();
	...
	// Get SVG.
	$svg = apply_filters( 'lilo_get_social_svg', lilo_get_social_svg( $icon ), $item_output );
...
add_filter( 'walker_nav_menu_start_el', 'lilo_social_icons_menu', 10, 4 );
 ...
 */
function lilo_supported_social_icons() {
	...
	return apply_filters( 'lilo_supported_social_icons', $supported_social_icons );
}
