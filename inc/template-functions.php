...
function lilo_body_classes( $classes ) {
	...
	if ( lilo_has_sidebar() && 'left-sidebar' === $theme_options['sidebar_position'] &&
 ...
 * @return bool
 */
function lilo_has_sidebar() {
	if ( ! is_active_sidebar( 'sidebar-1' ) ) {
		return false;
	}

	if ( lilo_is_blog_page() && ! is_page_template( 'templates/template-no-sidebar.php' ) ) {
		return true;
	}
 ...
 * @return void
 */
function lilo_retina_logo() {
	// Return early if there is no logo image or option for retina logo is disabled.
	if ( ! has_custom_logo() or false === lilo_get_option( 'retina_logo' ) ) {
		return;
	}
	...
	// Add Custom CSS.
	wp_add_inline_style( 'lilo-stylesheet', $css );
}
add_filter( 'wp_enqueue_scripts', 'lilo_retina_logo', 11 );
 ...
 * @return int
 */
function lilo_excerpt_length( $length ) {
	...
	$excerpt_length = lilo_get_option( 'excerpt_length' );
...
add_filter( 'excerpt_length', 'lilo_excerpt_length' );
 ...
 * @return string
 */
function lilo_excerpt_more( $more_text ) {

	if ( is_admin() ) {
		return $more_text;
	}

	return esc_html( ' ' . lilo_get_option( 'excerpt_more_text' ) );
}
add_filter( 'excerpt_more', 'lilo_excerpt_more' );
 ...
 */
function lilo_get_featured_post_ids( $cache_id, $category, $number_of_posts ) {

	$cache_id = sanitize_key( $cache_id );
	$post_ids = get_transient( 'lilo_featured_post_ids' );
	...
	if ( ! isset( $post_ids[ $cache_id ] ) || is_customize_preview() ) {
		...
		// Set Transient.
		set_transient( 'lilo_featured_post_ids', $post_ids );
	}

	return apply_filters( 'lilo_featured_post_ids', $post_ids[ $cache_id ], $cache_id );
}
 ...
 * @return void
 */
function lilo_flush_featured_post_ids() {
	delete_transient( 'lilo_featured_post_ids' );
}
add_action( 'save_post', 'lilo_flush_featured_post_ids' );
add_action( 'deleted_post', 'lilo_flush_featured_post_ids' );
add_action( 'customize_save_after', 'lilo_flush_featured_post_ids' );
add_action( 'switch_theme', 'lilo_flush_featured_post_ids' );
 ...
 * @return array Post IDs
 */
function lilo_exclude_featured_post_ids_from_blog( $query ) {
	...
	if ( $query->is_home() && $query->is_main_query() && ! is_admin() ) {
		...
		$featured_category = lilo_get_option( 'featured_category' );
		$featured_layout   = lilo_get_option( 'featured_layout' );

		// Get cached post ids.
		$post_ids = lilo_get_featured_post_ids( 'featured-content', $featured_category, $featured_layout );

		// Exclude Posts.
		$query->set( 'post__not_in', $post_ids );
	}
}
add_action( 'pre_get_posts', 'lilo_exclude_featured_post_ids_from_blog' );
